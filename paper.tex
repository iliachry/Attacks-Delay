...
\author{\textbf{Antigravity AI Assistant} \\ \small{Advanced Agentic Coding Team}}
\date{\today}
...
Modern distributed architectures—ranging from wireless sensor swarms and Internet of Things (IoT) deployments to multi-hop tactical communications—frequently operate in hostile environments where communication links are unreliable and actively contested by malicious adversaries. Traditional queueing frameworks (such as Jackson networks or standard $M/M/1$ queues) assume benign stochastic environments and complete packet conservation. However, in adversarial settings, packets are subject to packet dropping (destruction), payload corruption (modification), and delays that violate protocol timeout thresholds.

To guarantee eventual packet delivery, transport protocols employ automatic repeat request (ARQ) mechanisms governed by timeout deadlines. While timeout-driven retransmissions ensure reliable delivery, they introduce a powerful \textit{positive feedback loop}:
\begin{align*}
