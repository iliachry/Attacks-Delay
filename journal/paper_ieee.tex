...
\title{Mathematical Modeling and Performance Analysis of Multi-Node Queueing Networks Under Adversarial Attacks and Timeout-Driven Retransmissions}

\author{Ilias~Chrysovergis%
\IEEEcompsocitemizethanks{
\IEEEcompsocthanksitem I.~Chrysovergis is with the Department of Electrical and Electronic Engineering, Imperial College London, London SW7 2AZ, UK, and Metatopia. E-mail: ilias@chrysovergis.gr.
}}

\markboth{IEEE Transactions on Networking,~Vol.~XX, No.~X, August~2026}%
{Chrysovergis \MakeLowercase{\textit{et al.}}: Mathematical Modeling of Queueing Networks Under Adversarial Attacks}
...
\begin{IEEEkeywords}
Queueing theory, adversarial networks, renewal-reward process, fixed-point traffic analysis, hypoexponential distributions, moment-matching, Erlang distribution, network stability.
\end{IEEEkeywords}}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.82\textwidth]{fig1_system_model.png}
...
In this paper, we develop a unified, comprehensive theoretical foundation to model end-to-end sojourn times and stability boundaries across five distinct case studies:
\begin{itemize}
...
\end{equation}
Each packet undergoes $A - 1$ failed attempts followed by exactly one successful attempt. Applying the renewal-reward theorem \cite{ross1996stochastic}, the total end-to-end sojourn time $D$ is:
\begin{equation}
...
In a pre-service destruction attack, an adversary intercepts and destroys the packet before it receives service from the server with probability $p$. If destroyed, the packet consumes no server processing time, but the time spent waiting in the queue is wasted, and the packet undergoes a deterministic backoff $B$ before re-entering the queue. If the packet is not attacked (probability $1 - p$), it enters the server and is processed with exponential service time $S \sim \text{Exp}(\mu)$. The timeout threshold $T$ is checked against the total service phase $(W_q + S)$.
...
For an $M/M/1$ queue with parameter $\gamma = \mu - \lambda_{\text{eff}}$, the CDF of the total time in system $W_q + S$ is:
\begin{equation}
...
\begin{align}
P_{\text{succ}} &= (1 - p) P(W_q + S \le T) \nonumber \\
&= (1 - p)\left(1 - \frac{\Lambda^*(1-p)}{\mu} e^{-(\mu - \Lambda^*(1-p))T}\right)
\end{align}
The expected delay per attempt is:
\begin{align}
E[D_{\text{attempt}}] &= E[W_q] + (1 - p) E[S] \nonumber \\
&= \frac{\Lambda^*(1-p)}{\mu(\mu - \Lambda^*(1-p))} + \frac{1-p}{\mu}
\end{align}
The total end-to-end sojourn time is:
\begin{equation}
E[D] = \frac{1}{P_{\text{succ}}} \left[E[W_q] + \frac{1-p}{\mu}\right] + \left(\frac{1}{P_{\text{succ}}} - 1\right) B
\end{equation}
...
\begin{itemize}
    \item \textbf{Destruction Attacks (Case 1):} Server utilization depends only on surviving traffic $\Lambda^*(1-p)$. The system stability condition is $\Lambda^*(1-p) < \mu \implies \Lambda^* < \frac{\mu}{1-p}$.
...
\end{itemize}
This theoretical divergence is confirmed in Fig. \ref{fig:single_node}.
...
In a multi-hop feedforward network, a packet must traverse a sequence of $N$ distinct nodes in series: Node $0 \to \text{Node } 1 \to \dots \to \text{Node } (N-1)$. Let $\Lambda_j^*$ be the arrival rate at node $j$, and let $\mu_j = \mu$ be the service rate. By Burke's theorem \cite{burke1956output}, each node $j$ operates as an $M/M/1$ queue with sojourn time $X_j \sim \text{Exp}(\mu - \Lambda_j^*)$:
\begin{equation}
E[X_j] = \frac{1}{\mu - \Lambda_j^*}, \quad \text{Var}(X_j) = \frac{1}{(\mu - \Lambda_j^*)^2}
\end{equation}
The total continuous journey time across $N$ nodes is the convolution of $N$ independent exponential random variables, known as a \textbf{Hypoexponential distribution} \cite{cox1955use}:
\begin{equation}
S_N = \sum_{j=0}^{N-1} X_j
\end{equation}
The exact mean and variance of $S_N$ are:
\begin{equation}
\mu_S = \sum_{j=0}^{N-1} \frac{1}{\mu - \Lambda_j^*}, \quad \sigma_S^2 = \sum_{j=0}^{N-1} \frac{1}{(\mu - \Lambda_j^*)^2}
\end{equation}
We employ a two-parameter \textbf{Gamma moment-matching approximation} \cite{altiok1985approximate} $S_N \sim \text{Gamma}(k, \theta)$ with shape $k = \mu_S^2/\sigma_S^2$ and scale $\theta = \sigma_S^2/\mu_S$:
\begin{equation}
...
Under symmetric load across stages ($\Lambda_0^* = \Lambda_1^* = \Lambda_2^* = \Lambda^*$), the distribution $S_3$ reduces to an exact Erlang-3 distribution. The overall success probability is:
\begin{equation}
...
\begin{equation}
T_i = \frac{1}{\mu - \Lambda_0^*(1 - p)^i}, \quad \text{Var}(T_i) = \frac{1}{[\mu - \Lambda_0^*(1 - p)^i]^2}
\end{equation}
The total journey statistics are $\mu_S = \sum_{i=0}^{N-1} T_i$ and $\sigma_S^2 = \sum_{i=0}^{N-1} T_i^2$. The end-to-end success probability for an attempt is:
\begin{equation}
P_{\text{succ}} = (1 - p)^N p_{\text{no\_to}}, \quad p_{\text{no\_to}} = \frac{\gamma(k, W/\theta)}{\Gamma(k)}
\end{equation}

\subsubsection{Truncated Conditional Delays and Failure Decomposition}
We compute the conditional expectations of successful and failed attempts:
\begin{enumerate}
    \item \textbf{Successful Attempt Duration $E[D_{\text{succ}}]$:} Conditioned on $S_N \le W$, the expectation is given by the truncated Gamma moment:
    \begin{equation}
    E[D_{\text{succ}}] = \mu_S \frac{\gamma(k+1, W/\theta)}{\gamma(k, W/\theta)}
    \label{eq:truncated_gamma_succ}
    \end{equation}
    \item \textbf{Failed Attempt Duration $E[D_{\text{fail}}]$:}
    \begin{equation}
    E[D_{\text{fail}}] = \frac{\sum_{i=0}^{N-1} w_{\text{att}, i} \left(\sum_{j=0}^i T_j\right) + w_{\text{to}} E[S_N \mid S_N > W]}{\sum_{i=0}^{N-1} w_{\text{att}, i} + w_{\text{to}}}
    \end{equation}
    where $w_{\text{att}, i} = (1-p)^i p$, $w_{\text{to}} = (1-p)^N (1 - p_{\text{no\_to}})$, and $E[S_N \mid S_N > W] = \mu_S \frac{1 - \frac{\gamma(k+1, W/\theta)}{\Gamma(k+1)}}{1 - \frac{\gamma(k, W/\theta)}{\Gamma(k)}}$.
\end{enumerate}
...
Because Node 0 bears the full burden of retransmissions ($\Lambda_0^* = \lambda / P_{\text{succ}}$), it is the bottleneck node. As $N$ grows, delay increases super-linearly (Fig.~\ref{fig:tandem}). Analysis across the operational domain ($\lambda, p \in [0.05, 0.30]$) confirms that the system preserves a robust, 100\% stable operating envelope for all $N \in \{2, 3, 4\}$, with bottleneck utilization remaining strictly subcritical ($\rho_0 < 1$) throughout (Fig.~\ref{fig:stability}). Global throughput satisfies $\Gamma_{\text{dest}} = \lambda$.

\begin{figure}[t]
    \centering
    \begin{subfigure}[b]{0.24\textwidth}
        \includegraphics[width=\textwidth]{4_n_node_feedforward/section_3_3_2_tandem_delay_vs_N_varying_p.png}
        \caption{Delay vs. $N$}
        \label{fig:tandem}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.24\textwidth}
        \includegraphics[width=\textwidth]{4_n_node_feedforward/section_3_3_2_stability_regions.png}
        \caption{Stability envelope}
        \label{fig:stability}
    \end{subfigure}
    \caption{Multi-hop feedforward dynamics (Case 4). Left: Non-linear delay growth with $N$. Right: Robust 100\% stable operating envelope across $(\lambda, p \in [0.05, 0.30])$ for $N \in \{2, 3, 4\}$.}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item With probability $1 - q = \frac{N}{N+1}$, the packet transitions uniformly to any of the $N$ nodes ($r_{ij} = \frac{1}{N+1}$).
    \end{itemize}
\end{itemize}
By complete topological and traffic symmetry, all nodes maintain an identical total arrival rate $\Lambda^*$. The effective node clearance rate is $\gamma = \mu - \Lambda^*$, and the sojourn time per node visit is $X \sim \text{Exp}(\gamma)$.

\subsection{Erlang-$k$ Multi-Visit Sojourn Distribution}
The cumulative time required to complete $k$ successive node visits follows an \textbf{Erlang-$k$} distribution $S_k \sim \text{Erlang}(k, \gamma)$:
\begin{equation}
F_k(W) = P(S_k \le W) = \frac{\gamma(k, \gamma W)}{\Gamma(k)} = 1 - \sum_{j=0}^{k-1} \frac{(\gamma W)^j e^{-\gamma W}}{j!}
\end{equation}
with $F_0(W) = 1$. Let the reach weight be $u_k = (1 - p)^{k-1} \left(\frac{N}{N+1}\right)^{k-1} F_{k-1}(W)$.
...
\begin{enumerate}
    ...
    \end{equation}
    \item \textbf{Success Probability $P_{\text{succ}}$:}
    \begin{equation}
    ...
    \begin{equation}
    E[D_{\text{succ}}] = \frac{1}{P_{\text{succ}}} \sum_{k=1}^\infty P_{\text{succ}, k} \left(\frac{k}{\gamma} \frac{F_{k+1}(W)}{F_k(W)}\right)
    \end{equation}
    ...
    \end{equation}
    where $P_{\text{att}, k} = u_k p$ and $P_{\text{to}, k} = u_k (1 - p) [F_{k-1}(W) - F_k(W)]$.
\end{enumerate}

\subsection{Fixed-Point Traffic Equation and Delay}
The fixed-point traffic balance at every node is:
\begin{equation}
\Lambda^* = \lambda \frac{E[V](\Lambda^*)}{P_{\text{succ}}(\Lambda^*)}
\label{eq:feedback_fixed_point}
\end{equation}
Solving Eq. \eqref{eq:feedback_fixed_point} via damped Picard iteration yields $\Lambda^*$. The total end-to-end sojourn time is:
\begin{equation}
E[D] = \left(\frac{1}{P_{\text{succ}}} - 1\right) E[D_{\text{fail}}] + E[D_{\text{succ}}]
\end{equation}
Fig. \ref{fig:feedback} illustrates the scaling of sojourn time with network size $N$.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.45\textwidth]{5_n_node_feedback/section_3_3_1_sojourn_vs_N_varying_p.png}
...
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}lp{3.5cm}p{4.8cm}p{6.5cm}@{}}
\toprule
...
\textbf{Case 1: Destruction} & $N=1, \mu=10, \lambda=1.44, p=0.2, W=2, B=0.03$ & 0.1286 & 0.1261 & 0.0025 & 1.91\% \\
\textbf{Case 2: Modification} & $N=1, \mu=10, \lambda=1.44, p=0.2, W=2$ & 0.1525 & 0.1523 & 0.0002 & 0.15\% \\
\textbf{Case 3: 3-Node Tandem} & $N=3, \mu=2.0, \lambda=0.15, p=0.10, W=8$ & 2.0511 & 2.0463 & 0.0048 & 0.23\% \\
\textbf{Case 4: General Feedforward} & $N=4, \mu=2.0, \lambda=0.15, p=0.10, W=8$ & 2.9124 & 2.9079 & 0.0045 & 0.15\% \\
\textbf{Case 5: Feedback Mesh} & $N=10, \mu=1.0, \lambda=0.05, p=0.10, W=50$ & 31.6064 & 31.8035 & 0.1971 & 0.62\% \\ \bottomrule
\end{tabular}
\label{tab:validation}
\end{table*}

\section{Comparative Theoretical Synthesis}
Table \ref{tab:theory_synthesis} synthesizes the theoretical formulations across all five case studies.

\section{Experimental Validation}
The proposed analytical models were validated against discrete-event simulations implemented in SimPy \cite{matloff2008introduction}. Simulations were executed with 80–200 replications, a warm-up period of 2,000 time units, and an active observation horizon of 20,000 time units per replication. 

As shown in Table \ref{tab:validation}, theoretical predictions align with simulation data across all five case studies with relative errors under 1\% in multi-node scenarios and under 2\% in single-node cases.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{fig1_system_model.png}}]{Ilias Chrysovergis}
is a researcher and system architect at Metatopia. His research interests encompass distributed systems, communication network resilience, stochastic queueing theory, and security protocol engineering in adversarial environments. E-mail: ilias@chrysovergis.gr.
\end{IEEEbiography}
