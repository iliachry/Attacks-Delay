...
def solve_feedback_network_theory(N, mu, lambda_arr, p, W):
    ...
    for _ in range(300):
        ...
        if abs(next_val - lambda_star_sol) < 1e-6:
    ...
    def sim_func():
        ...
        def packet_generator(env, node_id):
            ...
            while True:
                ...
                p = NetworkPacket(f"N{node_id}P{packet_counter}", node_id, env.now)
                queues[node_id].put(p)
                packet_counter += 1
...
def plot_sojourn_time_vs_N_varying_p():
    ...
    for idx, p in enumerate(p_values):
        ...
        for N in N_values:
            ...
            if avg_sojourn is not None and avg_sojourn != np.inf:
                sim_times.append(run_multiple_simulations(N, mu, lambda_arrival, p, W, replications))
            else:
    ...
    plt.title('Feedback Network - Average Sojourn Time vs N')
    plt.legend()
    plt.grid(True)
